...
During the implementation, a _feature id_ will be required to identity the new
feature in the code base to avoid the new functionality until its activation.
The _feature id_ for a feature proposal is derived by running the following
commands.
...
Now run the `spl-feature-proposal` program to derive the _feature id_:
...
the _feature id_ will be visible in `solana feature status` and the _feature
proposer_ may initiate the community proposal process.
...
**COST:** As a part of token distribution, the _feature proposer_ will be
